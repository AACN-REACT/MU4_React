export function XHRNew(item, url, setter, dispatch, token, setErrorMsg) {
  ...
  xhr.addEventListener("readystatechange", (e) => {
    if (xhr.readyState == 4 && xhr.status == 200) {
      if(xhr.response['Result']===false){ console.log("...", setErrorMsg);setErrorMsg(xhr.response['Errors'].length>0?xhr.response['Errors'][0]['Value']:xhr.response['ErrorInfo'].UserErrorMessage?xhr.response['ErrorInfo'].UserErrorMessage:"Error occured")}
